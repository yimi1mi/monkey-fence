...
impl WindowsWindowInner {
    ...
    fn handle_ime_composition_inner(&self, ctx: HIMC, lparam: LPARAM) -> Option<isize> {
        ...
        if lparam == 0 {
            ...
            if lparam & GCS_RESULTSTR.0 > 0 {
                ...
                let text = String::from_utf16_lossy(&comp_result);
                if self
                    ...
                    .is_none()
                {
                    // MonkeyFence 补丁:无 InputHandler 的自绘输入框(读
                    // key_char 的 on_key 处理器)也要能收到 IME 上屏文本。
                    // 合成为 key_char KeyDown 事件,走 WM_KEYDOWN 同一分发
                    // 通道。上游行为(有 InputHandler 时)保持不变。
                    self.dispatch_ime_commit_as_keydown(&text);
                }
            }
    ...
    /// MonkeyFence 补丁:把 IME 上屏文本作为 KeyDown 事件分发给应用
    /// (keystroke.key = "ime",key_char = 完整上屏串),供无 InputHandler
    /// 的元素级 on_key 文本输入路径使用。
    fn dispatch_ime_commit_as_keydown(&self, text: &str) {
        if text.is_empty() {
            return;
        }
        let Some(mut func) = self.state.callbacks.input.take() else {
