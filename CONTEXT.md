...
**节点输入（Node Input）:**
一次智能体执行将接收的业务内容及其来源。用户可在发送前覆盖本次输入；已发送内容只读，覆盖不改变上游原始交接。
_避免_: prompt 缓存、前端状态

**输入映射（Input Binding）:**
下游节点对某个上游交接字段的具名引用，声明字段是否必填以及可选默认值。它与直接模板引用共同决定节点需要哪些上游内容。
_避免_: 数据管道、字段订阅

**输入试算（Input Preview）:**
用节点模板和示例交接计算预期输入的只读预演，不启动智能体，也不改变工作流或正式待发送内容。
_避免_: 预览会话、工作流运行

**输入检查门控（Input Review Gate）:**
节点在启动前等待用户查看、调整并确认输入的介入点。用户要求人工检查或必填输入未满足时，确认前都不产生新的智能体执行。
_避免_: 前端弹窗、临时暂停

**暂停派发（Pause Dispatch）:**
工作流运行暂时停止启动新智能体执行的状态；正在工作的智能体仍可结算。重试和修改运行图不会隐式解除暂停。
_避免_: 全局停止、进程挂起

**图补丁（Graph Patch）:**
暂停期间对尚未启动节点的成组调整，生效为新的流水线版本。已启动节点保持完整冻结定义，其执行和结果由新版本继承。
_避免_: 草稿覆盖、改写历史
**结构化交接（Handoff）**:
