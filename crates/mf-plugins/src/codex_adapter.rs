//! Codex Adapter:每次运行独立的 `CODEX_HOME`。
//!
//! OpenAI Codex CLI 的配置加载器读取 `CODEX_HOME` 环境变量
//! (未设置时默认 `~/.codex`,配置文件为 `config.toml`;
//! 见 codex 仓库 codex-rs 的配置加载实现)。
//! 本适配器只把该变量指向本次 Agent Run 的 run-temp 子目录,
//! 并物化实例快照声明的配置;绝不读写用户真实 `~/.codex`。
...
const CONFIG_ENV: &str = "CODEX_HOME";
const CONFIG_SUBDIR: &str = "codex";

pub struct CodexAdapter;

impl CodexAdapter {
    pub fn new() -> CodexAdapter {
        CodexAdapter
    }
}
