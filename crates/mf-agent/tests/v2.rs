...
use mf_agent::store::{Migration, Store};
...
#[test]
fn migration_fresh_then_repeat_is_noop() {
    ...
    store2.run_migrations().unwrap();
    let versions: Vec<i64> = store2
        .with_conn(|c| {
...
fn migration_failure_rolls_back() {
    ...
    let db = tmp.path().join("orchestration.db");
    let store = Store::open(&db).unwrap();
    // 注入一个中途失败的迁移:先建表写入,再执行非法 SQL
    let bad: Vec<Migration> = vec![Migration {
        version: 99,
        name: "bad",
        up: |tx| {
            tx.execute_batch(
                "CREATE TABLE rollback_probe (id INTEGER); INSERT INTO rollback_probe VALUES (1);",
            )?;
            tx.execute_batch("THIS IS NOT SQL;")
                .map_err(|e| anyhow::anyhow!("bad sql: {e}"))?;
            Ok(())
        },
    }];
    assert!(store.run_migrations_with(bad).is_err());
    // 迁移失败:表与其中的行都不应存在,版本未记录
    let probe_exists: bool = store
        .with_conn(|c| {
            Ok(c.query_row(
        ...
        .unwrap();
    assert!(!probe_exists, "失败迁移的部分写入必须回滚");
    let v99: bool = store
        .with_conn(|c| {
            Ok(c.query_row(
...
/// 旧库(v1 数据)迁移:runs→Task、tasks→Step、dispatches→Agent Run,旧数据保留。
#[test]
fn migration_from_legacy_preserves_data() {
    let tmp = tempfile::tempdir().unwrap();
    let db = tmp.path().join("orchestration.db");
    {
        // 用 v1 schema + 直接 SQL 播种旧数据
        let conn = rusqlite::Connection::open(&db).unwrap();
        conn.execute_batch(
            "CREATE TABLE runs (id INTEGER PRIMARY KEY AUTOINCREMENT, objective TEXT NOT NULL,
                status TEXT NOT NULL DEFAULT 'active', created_at TEXT NOT NULL);
             CREATE TABLE tasks (id INTEGER PRIMARY KEY AUTOINCREMENT, run_id INTEGER NOT NULL,
                parent_id INTEGER, spec TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'pending',
                deps TEXT NOT NULL DEFAULT '[]', result TEXT, failure_count INTEGER NOT NULL DEFAULT 0,
                created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
             CREATE TABLE dispatches (id INTEGER PRIMARY KEY AUTOINCREMENT, task_id INTEGER NOT NULL,
                status TEXT NOT NULL DEFAULT 'dispatched', worker TEXT NOT NULL DEFAULT '',
                started_at TEXT NOT NULL, ended_at TEXT);
             CREATE TABLE messages (id INTEGER PRIMARY KEY AUTOINCREMENT, run_id INTEGER NOT NULL,
                from_handle TEXT NOT NULL, to_handle TEXT NOT NULL, kind TEXT NOT NULL,
                body TEXT NOT NULL, created_at TEXT NOT NULL);
             CREATE TABLE questions (id INTEGER PRIMARY KEY AUTOINCREMENT, run_id INTEGER NOT NULL,
                task_id INTEGER, question TEXT NOT NULL, answer TEXT, status TEXT NOT NULL DEFAULT 'open',
                created_at TEXT NOT NULL);
             INSERT INTO runs (objective, status, created_at) VALUES ('旧目标A', 'done', '2026-01-01T00:00:00Z');
             INSERT INTO runs (objective, status, created_at) VALUES ('旧目标B', 'active', '2026-01-02T00:00:00Z');
             INSERT INTO tasks (run_id, spec, status, deps, created_at, updated_at)
                VALUES (1, '旧任务1', 'completed', '[]', '2026-01-01T01:00:00Z', '2026-01-01T01:00:00Z');
             INSERT INTO tasks (run_id, spec, status, deps, created_at, updated_at)
                VALUES (1, '旧任务2', 'pending', '[1]', '2026-01-01T02:00:00Z', '2026-01-01T02:00:00Z');
             INSERT INTO dispatches (task_id, status, started_at) VALUES (1, 'completed', '2026-01-01T01:00:00Z');
             INSERT INTO dispatches (task_id, status, started_at) VALUES (2, 'dispatched', '2026-01-01T02:00:00Z');
             INSERT INTO messages (run_id, from_handle, to_handle, kind, body, created_at)
                VALUES (1, 'planner', 'coordinator', 'status', '规划完成', '2026-01-01T00:00:00Z');
             INSERT INTO questions (run_id, task_id, question, status, created_at)
                VALUES (1, 1, '继续吗?', 'answered', '2026-01-01T00:00:00Z');",
        )
        .unwrap();
    }
    let store = Store::open(&db).unwrap();
    ...
    let done_task = tasks.iter().find(|t| t.title == "旧目标A").unwrap();
    assert_eq!(done_task.status, TaskStatus::Succeeded);
    let active_task = tasks.iter().find(|t| t.title == "旧目标B").unwrap();
    assert_eq!(active_task.status, TaskStatus::NeedsYou);

    // 旧 tasks → 新 Step(带依赖)
    let steps = store.task_steps(done_task.id).unwrap();
    assert_eq!(steps.len(), 2);
    let s1 = steps.iter().find(|s| s.step_key == "legacy-1").unwrap();
    let s2 = steps.iter().find(|s| s.step_key == "legacy-2").unwrap();
    assert_eq!(s1.status, StepStatus::Succeeded);
    assert_eq!(s1.deps.len(), 0);
    assert_eq!(s2.deps, vec![s1.id]);

    // 旧 dispatches → Agent Run
    let runs = store.list_runs_of_task(done_task.id).unwrap();
    assert_eq!(runs.len(), 2);
    assert!(runs.iter().any(|r| r.status == RunStatus::Succeeded));
    assert!(runs.iter().any(|r| r.status == RunStatus::Interrupted));
    ...
    let legacy_q: i64 = store
        .with_conn(|c| Ok(c.query_row("SELECT COUNT(*) FROM questions", [], |r| r.get(0))?))
        .unwrap();
    assert_eq!(legacy_q, 1);
}
