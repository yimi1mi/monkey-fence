//! legacy transport 客户端 adapter(T6a,Issue #37)。
//!
...
pub use mf_kernel::legacy_transport::{LegacyRequest, LegacyResponse, LEGACY_TRANSPORT_PROTOCOL};
...
mod tests {
    ...
    use mf_kernel::legacy_transport::LegacyTransportSession;
    use std::sync::Arc;
    ...
    impl CoreKernel for NoKernel {
        ...
        ) -> Result<mf_kernel::kernel::TerminalChannel, mf_kernel::kernel::KernelProblem> {
            Err(mf_kernel::kernel::KernelProblem::ServiceUnavailable(
                "no kernel".into(),
            ))
        }
        fn shutdown(
            ...
            mf_kernel::shutdown::ShutdownAssessment::default()
        }
    }
    ...
    fn client_and_server_round_trip_over_memory_pipe() {
        ...
        let server = std::thread::spawn(move || {
            mf_kernel::legacy_transport::serve_connection(&mut session, server_read, server_write)
        });
