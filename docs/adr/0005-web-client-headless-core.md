...
Web 客户端的一次性入口引导：Core 启动时生成一次性 nonce，完整入口 URL 经 stdout（`WEB_ENTRY=`）交给拉起者；同时在 per-user 状态目录（与 discovery.json 同目录，同受用户文件 ACL 保护）维护 `entry.url` 引导文件，内容始终是一个未消耗的一次性入口 URL——启动时写入，每次成功 `/auth/exchange` 消耗旧 nonce 后立即重签。托盘、launcher 等本地同用户进程读取该文件即可在任意时刻（包括浏览器会话失效后、Core 非自己拉起时）直达工作台。这不降低现有信任边界：Core 仅绑定回环地址，能读该文件的本地同用户进程本就能读取 Core 的 stdout 与状态库；nonce 仍然一次性消耗，文件即“当前有效的唯一新入口”。
